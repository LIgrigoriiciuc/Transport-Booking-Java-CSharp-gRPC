...
public class GrpcProxy : IReservationService
{
    ...
    public void Connect()
    {
        // insecure — Java server uses plaintext
    ...
    {
        var user = _client!.Login(new LoginPayload
            { Username = username, Password = password });

        // start push subscription after login
    ...
    {
        _pushCts?.Cancel(); // stop push stream first
    ...
    private void StartPushSubscription()
    {
        ...
        Task.Run(async () =>
        {
            try
            {
                var stream = _client!.SubscribeToPush(new Empty(),
                    cancellationToken: token);

                await foreach (var push in stream.ResponseStream
                                   .ReadAllAsync(token))
                {
